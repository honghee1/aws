		...
		<div class="row ipad-width">
			<div class="col-md-8 col-sm-12 col-xs-12">
				<div class="topbar-filter">
					<p>
						Found <span>${requestScope.pageMaker.totalCount } movies</span>in
						total
					</p>
				...
				<div class="flex-wrap-movielist">
					<c:forEach var="NowShowingMovieList"
						items="${requestScope.NowShowingMovieList }" varStatus="status">
						<div class="movie-item-style-2 movie-item-style-1">
							<img src="fileDown.do?fno=1&mcode=${NowShowingMovieList.mcode }"
								alt="">
							<div class="hvr-inner">
								<a href="/moviesingle.do?mcode=${NowShowingMovieList.mcode }">Read
							...
							<div class="mv-item-infor">
								...
								<p class="rate">
									<i class="ion-android-star"></i><span>${NowShowingMovieList.user_rating}</span>
								</p>
							</div>
						</div>
					</c:forEach>
					...
					<div class="pagination2">
						<ul class="btn-group pagination">
							<c:if test="${pageMaker.prev }">
								<li><a
									href='<c:url value="/NowShowingMovieList.do?page=${pageMaker.startPage-1 }"/>'><i
										class="fa fa-chevron-left"></i></a></li>
							</c:if>
							<c:forEach begin="${pageMaker.startPage }"
								end="${pageMaker.endPage }" var="pageNum">
								<li><a
									href='<c:url value="/NowShowingMovieList.do?page=${pageNum }"/>'><i
										class="fa">${pageNum }</i></a></li>
							</c:forEach>
							<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
								<li><a
									href='<c:url value="/NowShowingMovieList.do?page=${pageMaker.endPage+1 }"/>'><i
